...
use crate::types::{PyBaseObject, PyDict, PyModule, PyType};
...
impl<'p> Python<'p> {
    unsafe fn unchecked_downcast<T: PyTypeInfo>(self, ob: &PyBaseObject) -> &'p T {
        if T::OFFSET == 0 {
    ...
    unsafe fn unchecked_mut_downcast<T: PyTypeInfo>(self, ob: &PyBaseObject) -> &'p mut T {
        if T::OFFSET == 0 {
    ...
    pub unsafe fn from_borrowed_ptr_to_obj(self, ptr: *mut ffi::PyObject) -> &'p PyBaseObject {
...
mod test {
    ...
    use crate::types::{PyBaseObject, PyBool, PyDict, PyInt, PyList};
    use crate::Python;
    ...
    fn test_is_instance() {
        ...
        let py = gil.python();
        assert!(py
            .is_instance::<PyBool, PyBaseObject>(PyBool::new(py, true).into())
            .unwrap());
