...
pub trait FromPyObject<'source>: Sized {
    ...
    fn extract(ob: &'source PyBaseObject) -> PyResult<Self>;
}
...
{
    #[inline]
    default fn extract(ob: &'a PyBaseObject) -> PyResult<&'a T> {
        Ok(T::try_from(ob)?)
    }
}
...
{
    #[inline]
    default fn extract(ob: &'a PyBaseObject) -> PyResult<&'a mut T> {
...
where
    T: FromPyObject<'a>,
{
    fn extract(obj: &'a PyBaseObject) -> PyResult<Self> {
...
pub trait PyTryFrom<'v>: Sized {
    ...
    fn try_from<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v Self, PyDowncastError>;

    /// Cast from a concrete Python object type to PyObject. With exact type check.
    fn try_from_exact<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v Self, PyDowncastError>;

    /// Cast from a concrete Python object type to PyObject.
    fn try_from_mut<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v mut Self, PyDowncastError>;

    /// Cast from a concrete Python object type to PyObject. With exact type check.
    fn try_from_mut_exact<V: Into<&'v PyBaseObject>>(
        value: V,
    ) -> Result<&'v mut Self, PyDowncastError>;

    /// Cast a PyBaseObject to a specific type of PyObject. The caller must
    /// have already verified the reference is for this type.
    unsafe fn try_from_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v Self;

    /// Cast a PyBaseObject to a specific type of PyObject. The caller must
    /// have already verified the reference is for this type.
    #[allow(clippy::mut_from_ref)]
    unsafe fn try_from_mut_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v mut Self;
}

// TryFrom implies TryInto
impl<U> PyTryInto<U> for PyBaseObject
where
    U: for<'v> PyTryFrom<'v>,
{
...
where
    T: PyTypeInfo,
{
    fn try_from<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v T, PyDowncastError> {
    ...
    fn try_from_exact<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v T, PyDowncastError> {
        let value = value.into();
        unsafe {
    ...
    fn try_from_mut<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v mut T, PyDowncastError> {
        let value = value.into();
        unsafe {
            if T::is_instance(value) {
    ...
    #[inline]
    unsafe fn try_from_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v T {
        let value = value.into();
    ...
    #[inline]
    unsafe fn try_from_mut_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v mut T {
        let value = value.into();
        let ptr = if T::OFFSET == 0 {
