...
use crate::types::{PyBaseObject, PyType};
...
pub trait PyDescrProtocol<'p>: PyTypeInfo {
    fn __get__(&'p self, instance: &'p PyBaseObject, owner: Option<&'p PyType>) -> Self::Result
    where
    ...
    fn __set__(&'p self, instance: &'p PyBaseObject, value: &'p PyBaseObject) -> Self::Result
    where
    ...
    fn __delete__(&'p self, instance: &'p PyBaseObject) -> Self::Result
    where
    ...
    fn __set_name__(&'p self, instance: &'p PyBaseObject) -> Self::Result
    where
