...
use crate::types::{PyBaseObject, PyList, PyTuple};
...
impl PySequence {
    ...
    pub fn concat(&self, other: &PySequence) -> PyResult<&PySequence> {
        unsafe {
            let ptr = self
                .py()
                .from_owned_ptr_or_err::<PyBaseObject>(ffi::PySequence_Concat(
                    self.as_ptr(),
                    other.as_ptr(),
                ))?;
            Ok(&*(ptr as *const PyBaseObject as *const PySequence))
        }
    }
    ...
    pub fn repeat(&self, count: isize) -> PyResult<&PySequence> {
        unsafe {
            let ptr = self
                .py()
                .from_owned_ptr_or_err::<PyBaseObject>(ffi::PySequence_Repeat(
                    self.as_ptr(),
                    count as Py_ssize_t,
                ))?;
            Ok(&*(ptr as *const PyBaseObject as *const PySequence))
    ...
    #[inline]
    pub fn get_item(&self, index: isize) -> PyResult<&PyBaseObject> {
        unsafe {
            self.py()
    ...
    #[inline]
    pub fn get_slice(&self, begin: isize, end: isize) -> PyResult<&PyBaseObject> {
        unsafe {
    ...
    #[inline]
    pub fn set_slice(&self, i1: isize, i2: isize, v: &PyBaseObject) -> PyResult<()> {
        unsafe {
            err::error_on_minusone(
                self.py(),
...
where
    T: FromPyObject<'a>,
{
    default fn extract(obj: &'a PyBaseObject) -> PyResult<Self> {
...
{
    fn extract(obj: &'source PyBaseObject) -> PyResult<Self> {
...
fn extract_sequence<'s, T>(obj: &'s PyBaseObject) -> PyResult<Vec<T>>
where
    T: FromPyObject<'s>,
{
...
impl<'v> PyTryFrom<'v> for PySequence {
    fn try_from<V: Into<&'v PyBaseObject>>(value: V) -> Result<&'v PySequence, PyDowncastError> {
    ...
    fn try_from_exact<V: Into<&'v PyBaseObject>>(
        value: V,
    ) -> Result<&'v PySequence, PyDowncastError> {
        <PySequence as PyTryFrom>::try_from(value)
    }

    fn try_from_mut<V: Into<&'v PyBaseObject>>(
        value: V,
    ...
    #[inline]
    unsafe fn try_from_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v PySequence {
        let ptr = value.into() as *const _ as *const PySequence;
        &*ptr
    }

    #[inline]
    unsafe fn try_from_mut_unchecked<V: Into<&'v PyBaseObject>>(value: V) -> &'v mut PySequence {
