...
use crate::types::{PyBaseObject, PyList};
...
impl PyDict {
    ...
    pub fn get_item<K>(&self, key: K) -> Option<&PyBaseObject>
    where
        K: ToBorrowedObject,
    {
        key.with_borrowed_ptr(self.py(), |key| unsafe {
...
impl<'py> Iterator for PyDictIterator<'py> {
    type Item = (&'py PyBaseObject, &'py PyBaseObject);
...
impl<'a> std::iter::IntoIterator for &'a PyDict {
    type Item = (&'a PyBaseObject, &'a PyBaseObject);
