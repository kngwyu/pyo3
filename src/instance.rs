...
impl<'a, T> From<PyRef<'a, T>> for &'a PyBaseObject
where
    T: PyTypeInfo,
{
    fn from(pref: PyRef<'a, T>) -> &'a PyBaseObject {
        unsafe { &*(pref.as_ptr() as *const PyBaseObject) }
    }
}

impl<'a, T> From<PyRefMut<'a, T>> for &'a PyBaseObject
where
    T: PyTypeInfo,
{
    fn from(pref: PyRefMut<'a, T>) -> &'a PyBaseObject {
        unsafe { &*(pref.as_ptr() as *const PyBaseObject) }
    }
}
...
/// use pyo3::types::{PyDict, PyBaseObject};
///
/// pub fn get_dict_item<'p>(dict: &'p PyDict, key: &impl ToPyObject) -> Option<&'p PyBaseObject> {
