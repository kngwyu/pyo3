...
use pyo3::ffi;
use pyo3::prelude::*;
use pyo3::types::{PyBaseObject, PyBytes, PyDict, PySlice, PyString, PyType};
...
impl<'p> PyContextProtocol<'p> for ContextManager {
    ...
    fn __exit__(
        &mut self,
        ty: Option<&'p PyType>,
        _value: Option<&'p PyBaseObject>,
        _traceback: Option<&'p PyBaseObject>,
    ) -> PyResult<bool> {
        let gil = GILGuard::acquire();
...
impl<'p> PyMappingProtocol<'p> for Test {
    fn __getitem__(&self, idx: &PyBaseObject) -> PyResult<PyObject> {
        let gil = GILGuard::acquire();
